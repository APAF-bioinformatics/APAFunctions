...
getUniprotBatch = function(values=IDList, attributes=c("accession", "protein_name", "go_id")) {
  if (grepl("refGene", IDList[1]) == TRUE){
    ...
    for (i in 1:length(ID_edit)) {
      query = paste("https://rest.uniprot.org/uniprotkb/search?query=", ID_edit[i],
                    "&fields=", paste(attributes, collapse=","), "&format=tsv", sep="")
      res <- data.table::fread(text = getURI(query, .opts=curlOptions(followlocation=TRUE)), header = T, encoding = "UTF-8")
    ...
    for (i in 1:length(ID_edit_l)) {
      query = paste("https://rest.uniprot.org/uniprotkb/search?query=",
