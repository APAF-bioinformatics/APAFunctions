...
createResultsxlsx <- function(experiment = c("SWATH, TMT"), inputlist){ #Currently no defaults, you must supply all.
  ...
  if (experiment == "SWATH") {
    ...
    for (i in 1:length(res.list)) {
      if(length(is.na(res.list[[i]][,7]))==length(res.list[[i]][,7])){
        res.sheet <- inputlist[["res.list"]][[i]][,1:6]
        printxlsxDataSheet(wb, data=res.sheet, FCcol=grep("FC", names(res.list[[i]])), pvalcol=grep("pval", tolower(names(res.list[[i]]))), tabName=comps[i,1])
      } else {
